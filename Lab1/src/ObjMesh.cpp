...
//ObjMesh::ObjMesh(/*Vertex* vertices, unsigned int numVertices*/)
//{
...
ObjMesh::~ObjMesh()
{
	glDeleteVertexArrays(1, &m_vertexArrayObject); // deletes the arrays
}

//void ObjMesh::Init(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
//{
//	IndexedModel model;
//
//	for (unsigned int i = 0; i < numVertices; i++)
...
void ObjMesh::LoadModelFile(const std::string& fileName)
{
	IndexedModel model = OBJModel("..\\res\\models\\" + fileName + ".obj").ToIndexedModel(); // load model from file
	InitModel(model);
}

void ObjMesh::InitModel(const IndexedModel& model)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayBuffers[POSITION_VERTBUFF]); //tell opengl what type of data the buffer is (GL_ARRAY_BUFFER), and pass the data
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayBuffers[TEXCOORD_VERTBUFF]); //tell opengl what type of data the buffer is (GL_ARRAY_BUFFER), and pass the data
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_vertexArrayBuffers[INDEX_VERTBUFF]); //tell opengl what type of data the buffer is (GL_ARRAY_BUFFER), and pass the data
